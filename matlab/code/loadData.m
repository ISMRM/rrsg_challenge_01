...
range           = max( max(reshape(k, [], 2))-min(reshape(k, [], 2)) );
...
% Compute image resolution from k-space data if given in units 1/FOV
Nimg = ceil(2*max(abs(k(:))));
center = ceil(size(senseMaps,1)/2)-ceil(Nimg/2):ceil(size(senseMaps,1)/2)+ceil(Nimg/2)-1;
